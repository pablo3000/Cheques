...
/**
 * Convierte números a letras Fecha: 19/08/2010
 *
 * @author Pablo M.
 * @version 1.0
 */
public class ClsNum2Text {
    ...
    public String getText(int numero) {
        if (numero == 0) {
            return "cero";
        } else {
     ...
     * Obtiene el Texto
     *
     * @param numero Numero del tipo double
     * @param flag Especifica el formato de salida Ej: true=ocho con dos
     * false=ocho con 2/100
     * @return
     */
    public String getText(double numero, boolean flag) {

        String[] separados = "".valueOf(numero).split("\\.");
        int intParteEntera = Integer.parseInt(separados[0]);
        int intParteDecimal = Integer.parseInt(separados[1]);

        separados = null;

        if (flag) {
            String tempText = getText(intParteEntera) + " con " + intParteDecimal;
            if (intParteDecimal < 10 && intParteDecimal > -10) {
                tempText = tempText + "0";
            }
            return tempText + "/100";
        } else {
            if (intParteDecimal == 0) {
                return getText(intParteEntera);
            } else {
                return getText(intParteEntera) + " con "
                        + getText(intParteDecimal);
            }
        }
    ...
    /**
     * Igual que el getText(double,boolean), pero el boolean establecido a false
     *
     * @param numero Numero del tipo double
     * @return
     */
    public String getText(double numero) {
        return getText(numero, false);
    }

    /**
     * Obtiene el Texto
     *
     * @param numero Numero del tipo BigDecimal
     * @param flag Especifica el formato de salida Ej: true=ocho con dos
     * false=ocho con 2/100
     * @return
     */
    public String getText(BigDecimal numero, boolean flag) {
    ...
    /**
     * Igual que el getText(BigDecimal,boolean), pero el boolean establecido a
     * false
     *
     * @param numero Numero del tipo double
     * @return
     */
    public String getText(BigDecimal numero) {
        return getText(numero, false);
    }
}
